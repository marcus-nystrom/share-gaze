...
import threading
import my_Ring_Buffer as my_rb
import datetime
import numpy as np
...
class mySDK_class(threading.Thread):
    """
    ...
    def __init__(self, computer_setup='two_PC', port_listen=4444, port_send=5555):
        ...
        # Create 3 sample ringbuffer
        self.rb_t =  my_rb.RingBuffer(3)
        self.rb_x =  my_rb.RingBuffer(3)
        self.rb_y =  my_rb.RingBuffer(3)
    ...
    def connect(self):
        ...
        #print(res)
        #if res == 104:
            # "C:\Program Files (x86)\SMI\RED-m Configuration Tool\iViewREDm-Client.exe"
            #Start tracker!
...
#        print(''.join(["ET_EVB 1 ",image_name, " c:\\Marcus\\eyeimages\\"]))
    ...
    def run(self):
        ...
        while True:
            ...
            x = np.nan
            y = np.nan
            res, sampleData = self.get_sample()
            t = self.getTimestamp()
            #print(res,t,sampleData.rightEye.gazeX,sampleData.rightEye.gazeY)
            
            if res == 1 or res == 2:
    
                # Make sure the the samples are on the screen
                x = sampleData.rightEye.gazeX
                y = sampleData.rightEye.gazeY
            ...
            # No need to read values more frequently than twice the sampling frequency of the eye tracker
            core.wait(1.0/self.Fs/2)
